...
## تغییرات نسخه‌ی ۰.۸.۰

هسته به Aether 2.0.0 ارتقا یافته است. حالت MASQUE-in-MASQUE، تکه‌تکه کردن handshake در HTTP/2، پراکسی HTTP CONNECT و پراکسی بالادستی اضافه شده‌اند. مشکل نمایش پیشرفت، بررسی IPv6 و تلاش مجدد پس از لغو اتصال اصلاح شده است. تنظیمات Tor هنوز در رابط گرافیکی ارائه نمی‌شوند.

## نصب

جدیدترین نصب‌کننده را از [صفحه‌ی Releases](https://github.com/4H1R/Aether-GUI/releases) بردارید:
...
بسته‌های ویندوز ۶۴بیتی، macOS برای Apple Silicon و Intel، و لینوکس ۶۴بیتی در صفحه‌ی ریلیز قرار می‌گیرند.
...
۳. **گرفتن فایل باینری Aether**
   ...
   این دستور روی ویندوز، macOS و لینوکس کار می‌کند. نسخه و SHA-256 در `aether-core.json` مشخص شده‌اند؛ فایل اجرایی و پوشه‌ی `pt/` پس از بررسی در `src-tauri/binaries/core/` استخراج می‌شوند.
